...
library(dplyr)
library(varhandle)
activity_labels <-tbl_df(read.table("activity_labels.txt", sep=" "))
...
test_value<-cbind(test_subject,test_y, test_x)
...
train_value<-cbind(train_subject,train_y,train_x)

## Combine test dataset and training dataset
datall<-rbind(test_value,train_value)

## Extracts only the measurements on the mean and standarddeviation for each measurement.
data_mean_std <-datall[c(1,2,mean_std+2)]

## Uses descriptive activity names to name the activities in the data set
names(data_mean_std)[1:2]<-c("subjectID","activityID")
data_mean_std<-merge(activity_labels,data_mean_std, by="activityID")
